...
function launchSPARQLQuery(sparqlQuery, endpointGeneral, callback) {
    ...
    $.ajax({
        ...
        statusCode: {
            400: function(error) {
                alert("Error al lanzar la consulta. \n" + sparqlQuery + " \n");
            }
        },
...
function generateCheckBox(propData, resourceData, columName, div, posicion, endpointGeneral) {
    ...
    for (var i in propData) {
        ...
        propOntologyData.push({
            ontologia: valor,
            atributo: valorLimpio[valorLimpio.length - 1]
        });
    }


    createButton(div, function() {
        return buildQuery(resourceData, posicion, propOntologyData, endpointGeneral, 500);
    }, "Consultar en punto de consulta", "buttonbuildQuery", posicion);
    getTypesOfProperties(resourceData,propOntologyData,endpointGeneral,posicion);
}


function getTypesOfProperties(resourceData,propOntologyData, endpointGeneral,position) {
    //Tengo que lanzar 1 consulta con LIMIT 1 obteniendo una consulta con todos los atributos para asi
    //tener el tipo de dato.
    console.log("getTypesOfProperties");
    var checkboxes = document.getElementsByName("checkboxForm"+position);
    var resource = document.getElementById("resourceList" + position);
    var nameAttributes = [];
    ...
    for (var i = 0; checkboxes[i]; ++i) {
    	nameAttributes[i]="var" + checkboxes[i].value;
        query += "?var" + checkboxes[i].value + " ";
    }
    query += "where { \n";
    ...
    //ATRIBUTOS
    for (var j = 0; checkboxes[j]; ++j) {
        query += "?x ";
        // propOntologyData
        encontrado = false;
        var ontURL = null;
        for (i = 0; i < propOntologyData.length && !encontrado; i++) {
            if (propOntologyData[i].atributo == checkboxes[j].value) {
                ontURL = propOntologyData[i].ontologia;
                encontrado = true;
            }
        }
        if (encontrado)
            query += "<" + ontURL + "> ?var" + checkboxes[j].value + " . \n";
    }
    query += " }";
    query += "LIMIT 1";

    console.log("Enviando consulta para saber tipos...");
    var callbackSendQueryAttributes = function(data) {
        if (data !== null) { //Consulta CORRECTA
        	var divToMod=document.getElementById("propertiesDiv"+position);
        	for (var i = 0; i < nameAttributes.length; i++) {
        		divToMod.childNodes[i].childNodes[1].textContent+=" {"+data[0][nameAttributes[i]].type+"} ";
        		console.log(data[0][nameAttributes[i]].type);
        	}
            
        }
    };
    launchSPARQLQuery(query, endpointGeneral, callbackSendQueryAttributes);
...
function buildQuery(resourceData, position, propOntologyData, endpointGeneral, limit) {
